...
class CBaseSimpleControl(CBaseCommand):
    ...
    def Run(self):
        if self.protocol.role==BaseCommand.PV_ROLE_INTERNAL:
            ...
            with self.protocol.factory.lockDict:
                sb_protocol=self.protocol.factory.dictSuperbox.get(self.internalMessage.destId)
        ...
        with self.protocol.lockCmd:
            if self.protocol.role==BaseCommand.PV_ROLE_HUMAN:
                ...
                #check whether contain parameter superbox_id
                superbox_id=self.body.get(BaseCommand.PN_SUPERBOXID,None)
                ...
                interMessage.SetParam(InternalMessage.TTYPE_GATEWAY,superbox_id,0,InternalMessage.OPER_REQUEST,"",
                                  InternalMessage.TTYPE_HUMAN,self.protocol.client_id,id(self.protocol.transport))
                
                self.Send(interMessage)
                #threading.Timer(Config.timeout_superbox_control,self.timeout).start()
                reactor.callLater(Config.timeout_superbox_control,self.timeout)
    ...
    def timeout(self):
        self.Finish(BaseCommand.CS_SUPERBOXRESPTIMEOUT)
...
class CBaseRespSimpleControl(CBaseRespCommand):
    ...
    def Run(self):
        if self.protocol.role==BaseCommand.PV_ROLE_INTERNAL:
            account_protocol=self.protocol.factory.GetAccountProtocol(superbox_id=self.internalMessage.fromId,client_id=self.internalMessage.destId)
