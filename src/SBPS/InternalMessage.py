...
#online status,map of superbox_id-->CConnectionIterm
dictSuperboxServer={}
...
def CheckMapsByActiveTime():
    with lockMaps:
        for superbox_id in dictSuperboxServer.keys():
            if dictSuperboxServer[superbox_id].dt_active<datetime.datetime.now()-datetime.timedelta(seconds=Config.time_heartbeat):
                #timeout
                item=dictSuperboxServer.pop(superbox_id)
...
def LoadMapsFromDatabase():
    ...
    with SBDB.session_scope() as session :
        ...
        for superbox in session.query(SBDB_ORM.Superbox).filter(SBDB_ORM.Superbox.dt_active>datetime.datetime.now()-datetime.timedelta(seconds=Config.time_heartbeat)).all():
            item=CConnectionItem(superbox.server_id)
            item.dt_active=superbox.dt_active
            dictSuperboxServer[superbox.id]=item
            dictServerLoad[item.server_id]=dictServerLoad.get(item.server_id,0)+1
...
def RunOnlineMessage(message):
    global dictSuperboxServer, dictClientServer, dictServerLoad, dictServerInfo,lockMaps
    with lockMaps:
        ...
        if(message.destType[1]==TTYPE_HUMAN):
            ...
            if message.operation==OPER_ONLINE:
                ...
                if message.addition.lower()=='y':   #need redirect checking 
                    listSuperboxes=SBDB.GetSuperboxesByAccountId(message.fromId)
                    target_server_id=-1
                    for superbox_id in listSuperboxes:                        
                        if dictSuperboxServer.has_key(superbox_id):
                            #if there is a superbox connected to that account is connected on the same server with the account connection, don't redirect the account connection
                            if dictSuperboxServer[superbox_id].server_id==message.fromId:
                                return
                            else:
                                target_server_id=superbox_id
            ...
            if message.operation==OPER_ONLINE:
                if not dictSuperboxServer.has_key(message.destId):    
                    dictServerLoad[message.fromId]=dictServerLoad.get(message.fromId,0)+1
                    dictSuperboxServer[message.destId]=CConnectionItem(message.fromId)
                else:
                    dictSuperboxServer[message.destId].server_id=message.fromId
                    dictSuperboxServer[message.destId].dt_active=datetime.datetime.now()
            elif message.operation==OPER_OFFLINE:
                if dictSuperboxServer.has_key(message.destId):    
                    dictServerLoad[message.fromId]=dictServerLoad.get(message.fromId,0)-1
                    dictSuperboxServer.pop(message.destId)
        pass
