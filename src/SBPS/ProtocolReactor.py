...
#     
    ...
    def connectionLost(self, reason=twistedError.ConnectionDone):
        ...
        if self.role==Command.BaseCommand.PV_ROLE_HUMAN:
            ...
            InternalMessage.NotifyTerminalStatus(InternalMessage.TTYPE_GATEWAY, self.superbox_id, 0, InternalMessage.OPER_OFFLINE)
    ...
    def releaseFromDict(self):
        with self.factory.lockDict:
            ...
            if self.role == Command.BaseCommand.PV_ROLE_SUPERBOX:
                if self.factory.dictSuperbox.has_key(self.superbox_id):
                    if self.factory.dictSuperbox[self.superbox_id]==self:
                        self.factory.dictSuperbox.pop(self.superbox_id)
            elif self.role == Command.BaseCommand.PV_ROLE_HUMAN:
                for superboxId in SBDB.GetSuperboxIDsByAccountId(self.account_id):
                    if self.factory.dictAccounts.has_key(superboxId):
                        listAccount=self.factory.dictAccounts[superboxId]
                        if self in listAccount:
                            listAccount.remove(self)
                            if len(listAccount)<=0:
                                self.factory.dictAccounts.pop(superboxId)
...
class SBProtocolFactory(ServerFactory):
    ...
    def __init__(self):
        ...
        self.dictSuperbox={}    #key:superboxid,value:SBProtocol
        self.dictAccounts={}     #key:superboxid,value:array of SBProtocol
        self.lockDict=threading.RLock()
        self.SBMP_HEADERTAG=struct.pack("4B",0x02,0xAA,0x55,0x03)
        
        self.lockPendingCmd=threading.RLock()
       

    def GetAccountProtocol(self,superbox_id,client_id):
        with self.lockDict:
            if self.dictAccounts.has_key(superbox_id):
                for clientProtocol in self.dictAccounts[superbox_id]:
