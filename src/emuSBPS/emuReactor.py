...
class SBProtocol(Protocol):
    ...
    def AddDataAndDecode(self,lock,data):
        ...
        while len(self.m_buffer)>=Command.BaseCommand.CBaseCommand.HEAD_LEN :
            ...
            if command == None:
                break  
            if waitList.has_key(command.command_seq):
...
def SendAndVerify(requestCmd): 
    ...
    requestCmd.Send()
    bRet=False
    if waitLock(requestCmd.lock,requestCmd.cond,Config.timeout_superbox_control*5):
...
def Run():
    reactor.connectTCP("42.96.248.180",9630,SBProtocolFactory()) #@UndefinedVariable
